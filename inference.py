...
class EfficientADInferencer(torch.nn.Module):
    def __init__(
        self, teacher_path: str, student_path: str, autoencoder_path: str, device: Optional[str] = None
    ) -> None:
        super().__init__()
        self._device = device
        ...
        self._teacher_pdn = torch.load(teacher_path, map_location=self._device)
        self._student_pdn = torch.load(student_path, map_location=self._device)
        self._autoencoder = torch.load(autoencoder_path, map_location=self._device)

        self._channels = list(self._teacher_pdn.children())[-1].out_channels
    ...
    def forward(self, image: Union[torch.Tensor, Image]) -> torch.Tensor:
        with torch.no_grad():
            ...
            local_anomalies_map, _ = torch.max((teacher_result - student_result[:, : self._channels, :, :]) ** 2, dim=1)
            global_anomalies_map, _ = torch.max(
                (autoencoder_result - student_result[:, self._channels :, :, :]) ** 2, dim=1
            )
